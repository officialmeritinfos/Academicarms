...
class Login extends CI_Controller {
	...
	public function index($page='teachers/login')
	{
        ...
        if ($this->form_validation->run() == TRUE)
        {
        	...
        	if ($email_exists->num_rows() > 0) 
        	{
        		...
        		{
        			$session=sha1(time());
        			$session_data = array('teacherid' =>$email_exists->row()->Teacher_id,'Teacher_Session'=>$session);
        			...
        			else
        			{
                        ...
                        $set_teacher=$this->db->Set('TeacherSess',$session)->where('Teacher_id',$email_exists->row()->Teacher_id)->Update('Teacher');
                        if ($set_teacher) 
                        {
		...
		$data['pagename']='Teachers Login Page';
		$this->load->view($page,$data);
	}
	public function recoverpassword($page='teachers/login')
	{
		$general_settings=$this->Web_model->general_settings();
        ...
        if ($this->form_validation->run() == TRUE)
        {
        	...
        	if ($email_exists->num_rows()>0) 
        	{
        		$teacher_id=$email_exists->row()->Teacher_id;
        		$teacher_name=$email_exists->row()->Name;
                ...
                $msg="Dear $teacher_name, Your Password Reset Code is $code. Best Regards, $site_title";
                $recover_session=array('teacheremail' =>$email,'Set'=>TRUE);
		        $data_update = array('EmailCode' =>$code,'Email_code'=>$code_hashed );
		        $data_updates=$this->db->set($data_update)->where('Teacher_id',$teacher_id)->update('Teacher');
		        if ($data_updates) 
		        {
                	...
                	if ($this->session->has_userdata('Set')) 
		        	{
		                ...
		                redirect('teachers/login/verify_code');
		        	}
		        }
		        else
		        {
	...
	public function verify_code($page='teachers/verify_code')
	{
		if (!$this->session->has_userdata('Set')) 
		{
        ...
        if ($this->form_validation->run() == TRUE)
        {
        	$teacher_email=$this->session->userdata('teacheremail');
			$code=html_escape($this->input->post('code'));
			$sub_code=sha1($code);
			$code_exists=$this->db->select('*')->from('Teacher')->where('Email_code',$sub_code)->get();
			if ($code_exists->num_rows()>0) 
			{
				$data_update = array('Email_code' =>'','EmailCode'=>'');
				$data_updates=$this->db->set($data_update)->where('Teacher_id',$code_exists->row()->Teacher_id)->update('Teacher');
				if ($data_updates) 
				{
					$teachersess = array('teach_id' =>$code_exists->row()->Teacher_id,'Sets'=>TRUE);
					$teacher_sess=$this->session->set_userdata($teachersess);
					if ($this->session->has_userdata('Sets')) 
					{
		                ...
		                redirect('teachers/login/reset_password');
					}
				}
				else
				{
	...
	public function reset_password($page='teachers/reset_password')
	{
		if (!$this->session->has_userdata('Sets')) 
		{
		     //if session not started
        ...
        if ($this->form_validation->run() == TRUE)
        {
        	$teacher_id=$this->session->userdata('teach_id');
			$password=html_escape($this->input->post('password'));
			$data_updates=$this->db->set('Password',password_hash($password, PASSWORD_BCRYPT))->where('Teacher_id',$teacher_id)->update('Teacher');
			if ($data_updates) 
			{
					//if session is set
		        	$this->session->set_flashdata('message', "<div class=\"alert alert-success alert-dismissable\">
